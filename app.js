...
import {busResultMessage} from "./src/message";
...
app.post('/getBusArriveInformation', asyncHandler( async (req, res) => {
    ...
    if(arrmsg1 !== "운행종료" ) {
        switch (arrmsg1) {
            case "곧 도착":
                resultObject.description =  "이번 버스 : "+ getBusSize(veh1).label + "\n" + arrmsg1 + "\n";
                break;
            case "출발대기":
        ...
        if(arrmsg2 !=="출발대기") {
            ...
            let Station_result2 = split_Minute2[1].split('[')[1]
                .slice(0, -1)
                .replace("번째", "정류장");
            resultObject.description += "다음 버스 : " + getBusSize(veh2).label + "\n" + Station_result2 + ", " + split_Minute2[0] + "분 후 도착\n";
        }
        else{
            resultObject.description += "다음 버스 : " +arrmsg2;

        }
        resultObject.Image = "http://13.125.72.93/img/" + getBusImage(getBusSize(veh1).code, getBusSize(veh2).code);
